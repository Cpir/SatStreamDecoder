...
bool CNode_ZTS::init(t_ini& ini, string& init_name, CChain* pchain_arg)
{
    ...
    set_timestamp = false;
    dump_every = 0;
    /*setenv("TZ", "", 1)*/ _putenv_s("TZ", ""); _tzset(); //this is required to protect against local time zone correction
    //
...
void CNode_ZTS::do_frame_processing(CFrame* pf)
{
    ...
    if (((dump_every >1 ) && (c_counter % dump_every == 1))
        || (dump_every == 1))
    {
        time_t s= pzts->seconds;fpf_swap4bytes((BYTE*)&s);
        time_t ms = pzts->mkseconds;fpf_swap4bytes((BYTE*)&ms);
        char sz[40];tm* ptm;
