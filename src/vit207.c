...
#define Polys	Poly207		/* Select polynomials here */
#endif

/* Rate 1/2 codes */
unsigned int Poly207[] = { 0x6d, 0x4f };	 /* k=7; NASA and industry  */
...
int nn)                        // N
{
  ...
  enc207(symbols,data,nbytes,0,0); // Do the encoding
...
int enc207(unsigned char symbols[], unsigned char data[],
...
long *Metric              /* Final path metric (bigger is better) */
  ...
  long metric;
  vit207(&metric,ddec,symbols,*Nbits,mettab,0,0);
  *Metric=metric;
}
